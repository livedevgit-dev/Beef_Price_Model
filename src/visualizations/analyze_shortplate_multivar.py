# [파일 정의서]
# - 파일명: analyze_shortplate_multivar.py
# - 역할: 분석/시각화
# - 대상: 수입육 (삼겹양지)
# - 데이터 소스: USDA 도매가, 한국 도매가, 환율, 수입물량, 재고 데이터
# - 주요 기능: 가격(USD), 환율, 수급 데이터를 완벽히 분리(Decoupling)한 3단 대시보드 (삼겹양지 버전)
...
from src.config import DATA_RAW, EXCHANGE_RATE_XLSX, USDA_PLATE_USD_KG_CSV, MASTER_IMPORT_VOLUME_CSV, BEEF_STOCK_XLSX

def load_and_merge_shortplate_data():
    print("삼겹양지 다중 변수 데이터(가격, 환율, 수입물량, 재고)를 독립 변수로 병합 중입니다...")
    
    # 1. 미국 우삼겹 도매가 (순수 USD/kg)
    us_df = pd.read_csv(USDA_PLATE_USD_KG_CSV)
    us_df['date'] = pd.to_datetime(us_df['report_date'])
    us_df = us_df[['date', 'choice_usd_per_kg']].rename(columns={'choice_usd_per_kg': 'us_price_usd_kg'})
    ...
    vol_df = vol_df[['date', '부위별_양지_합계']].rename(columns={'부위별_양지_합계': 'import_volume'})
    vol_df.set_index('date', inplace=True)
    
    # 5. 재고량 (양지 전체)
    stock_df = pd.read_excel(BEEF_STOCK_XLSX)
    stock_df = stock_df[stock_df['부위별 부위별'].str.contains('양지', na=False)]
    ...
    # 데이터 병합 (2020년 이후 데이터만 추출하여 노이즈 축소)
    master_df = pd.concat([us_monthly, ex_monthly, kr_monthly, vol_df, stock_df], axis=1)
    master_df = master_df[master_df.index >= '2020-01-01']
    master_df = master_df.ffill().bfill()
    
    return master_df

def visualize_shortplate_multivar(df):
    ...
    ax1.set_title('삼겹양지 가격/환율/수급 독립 변수 분석 (2020~)', fontsize=14, fontweight='bold')
    ...
    bar1 = ax3.bar(df.index, df['import_volume'], width=20, color='tab:green', alpha=0.6, label='미국산 양지 수입물량')
    ax3.tick_params(axis='y', labelcolor='tab:green')
    
    ax3_twin = ax3.twinx()
    ax3_twin.set_ylabel('국내 양지 총 재고량', color='tab:orange', fontweight='bold')
    fill1 = ax3_twin.fill_between(df.index, 0, df['stock_volume'], color='tab:orange', alpha=0.2, label='국내 양지 총 재고량')
...
if __name__ == "__main__":
    try:
        shortplate_multivar_data = load_and_merge_shortplate_data()
        visualize_shortplate_multivar(shortplate_multivar_data)
